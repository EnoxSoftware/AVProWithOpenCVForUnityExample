...
namespace AVProWithOpenCVForUnityExample
{
    ...
    public class AVProVideoAsyncGPUReadback2MatHelperExample : MonoBehaviour
    {
        // Enums
        ...
        private void OnNewMediaReady()
        {
            ...
            // Update source texture and initialize
            if (MediaPlayer.TextureProducer != null)
            {
                _asyncGPUReadback2MatHelper.SourceTexture = MediaPlayer.TextureProducer.GetTexture();

                // Set FlipVertical based on MediaPlayer's RequiresVerticalFlip
                bool requiresVerticalFlip = MediaPlayer.TextureProducer.RequiresVerticalFlip();
                _asyncGPUReadback2MatHelper.FlipVertical = requiresVerticalFlip;
                FlipVerticalToggle.isOn = requiresVerticalFlip;
